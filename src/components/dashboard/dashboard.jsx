...
class Dashboard extends Component {
	...
	async componentDidMount() {
		...
		try {
			if (storage) {
				...
				const { data: { message: dog } } = await axios.get(
					`https://dog.ceo/api/breed/${breedsList[0]}/images/random`
				);

				const sortBreedList = breedsList.sort((a, b) => {
					return a - b;
				});

				this.setState({ breedsList: sortBreedList, loading: false, dog });
			}
		} catch (error) {
			console.log(error);
		}
	...
	handleChangeBreeds = async (breeds) => {
		try {
			const { data: { message } } = await axios.get(`https://dog.ceo/api/breed/${breeds}/images/random`);
	...
	render() {
		const { loading, breedsList, dog: imageDog, name, font, color } = this.state;
					...
					<div className="select-box">
						...
						<select onChange={(e) => this.handleChangeBreeds(e.target.value)}>
							{breedsList.map((dog, index) => {
								return <option key={index}>{dog.toLowerCase()}</option>;
							})}
						</select>
